...
import KeywordEvolution, {
  KeywordEvolutionRef,
} from "@/components/KeywordEvolution";
import CountryEvolution, {
  CountryEvolutionRef,
} from "@/components/CountryEvolution";
import InstitutionEvolution, {
  InstitutionEvolutionRef,
} from "@/components/InstitutionEvolution";
...
const IndexPage: React.FC = () => {
  ...
  const [paperListSortBy, setPaperListSortBy] = useState<"time" | "venue">(
    "time"
  );
  const [selectedEvolutionView, setSelectedEvolutionView] = useState<
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (years.length > 0) {
        ...
        setFilter((prev) => ({
          ...prev,
          years: [currentMinYear, currentMaxYear],
        }));
      }
    } catch (error) {
  ...
  const filteredPapers = useMemo(() => {
    ...
    if (filter.countries && filter.countries.length > 0) {
      currentPapers = currentPapers.filter(
        (paper) => paper.country && filter.countries!.includes(paper.country)
      );
    }

    if (filter.venues && filter.venues.length > 0) {
      currentPapers = currentPapers.filter((paper) =>
        filter.venues!.includes(paper.venue.name)
      );
    }

    if (filter.keywords && filter.keywords.length > 0) {
      // 将筛选关键词转换为小写用于匹配
      const filterKeywordsLower = filter.keywords.map((k) =>
        k.toLowerCase().trim()
      );
      currentPapers = currentPapers.filter(
        (paper) =>
    ...
    <Layout style={{ minHeight: "100vh" }}>
            ...
            <>
              ...
              {filter.keywords.map((keyword) => (
                <Tag
                  ...
                  closable
                  onClose={() => {
                    const newKeywords = filter.keywords!.filter(
                      (k) => k !== keyword
                    );
                    setFilter((prev) => ({
                      ...prev,
          ...
          <Col span={10}>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <Card
                title={
                  <span
                    style={{ display: "block", lineHeight: "16px", margin: 0 }}
                  >
                    动态交互词云
                  </span>
                }
                ...
                styles={{
                  ...
                  body: { padding: 0, flex: 1, display: "flex" },
                }}
              >
                <div style={{ flex: 1 }}>
                  <WordCloud
      ...
      {paperListVisible && (
        ...
        >
          ...
          >
            <div
              style={{ fontSize: "16px", fontWeight: "bold", color: "#e8f4f8" }}
            >
                ...
                >
                  ...
                  >
                    <div style={{ marginBottom: "4px" }}>
                      <span style={{ fontWeight: "500", color: "#e8f4f8" }}>
                        年份:
                      </span>{" "}
                      {paper.year}
                      {paper.venue?.name && (

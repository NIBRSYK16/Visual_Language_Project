...
const WordCloud: React.FC<WordCloudProps> = ({
  ...
  const drawWordCloud = useCallback(() => {
    ...
    svg.attr("width", width).attr("height", height).style("overflow", "hidden"); // 防止词云文本溢出容器
    ...
    const yearKey = filter.years
      ? `${filter.years[0]}-${filter.years[1]}`
      : "all";
    const shouldRecalculateLayout =
      !layoutCacheRef.current.has(yearKey) ||
      layoutCacheRef.current.size === 0 ||
      !wordData.every((d) =>
        layoutCacheRef.current.has(`${yearKey}-${d.word}`)
      );
    ...
    function draw(words: any[]) {
      ...
      const getColor = (d: any, isHover: boolean = false): string => {
        ...
        const frequencyRatio =
          (d.frequency - minFrequency) / (maxFrequency - minFrequency || 1);
      ...
      texts
        .on("mouseover", function (event, d: any) {
          ...
          const tempFilter = currentFilter.keywords?.includes(keywordToCheck)
            ...
            : {
                ...currentFilter,
                keywords: [...(currentFilter.keywords || []), keywordToCheck],
              };

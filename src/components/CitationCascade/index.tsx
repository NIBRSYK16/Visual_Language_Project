...
const CitationCascade: React.FC<CitationCascadeProps> = ({
  papers,
  filter,
}) => {
  ...
  const drawCascade = useCallback(() => {
    ...
    filteredPapers.forEach((paper) => {
      paper.references.forEach((refId) => {
        ...
        if (
          referencedPaper &&
          referencedPaper.year &&
          paper.year &&
          referencedPaper.year < paper.year
        ) {
    ...
    linkGroup
      ...
      .attr("d", (d) => {
        ...
        const sourceY =
          yearScale(String(sourceYear))! + yearScale.bandwidth() / 2;
        const targetY =
          yearScale(String(targetYear))! + yearScale.bandwidth() / 2;
    ...
    const statsGroup = g
      .append("g")
      .attr("class", "stats")
      .attr("transform", `translate(${chartWidth - 220}, 30)`);
  ...
  return (
    <div
      ref={containerRef}
      className="citation-cascade-container"

...
const InstitutionEvolution = forwardRef<InstitutionEvolutionRef, InstitutionEvolutionProps>(
  ({ papers, filter, onPlayStateChange, externalYear, externalIsPlaying }, ref) => {
    ...
    const drawChart = useCallback(
      (year: number | null) => {
        ...
        const yDomain = displayItems.map((d, i) => d ? d.institution : `_placeholder_${i}`);
        ...
        const colorScale = d3.scaleOrdinal(d3.schemeCategory10).domain(Array.from(allInstitutions));

        const tooltip = getTooltip();

        // 绘制柱状图（只绘制有数据的项）
        const barsData = displayItems.filter(d => d !== null) as InstitutionData[];
        const bars = g
        ...
        // 绘制标签（截断长名称，只绘制有数据的项）
        const labelsData = displayItems.filter(d => d !== null) as InstitutionData[];
        const labels = g
        ...
        // 绘制数值标签（只绘制有数据的项）
        const valueLabelsData = displayItems.filter(d => d !== null) as InstitutionData[];
        const valueLabels = g
    ...
    return (
      <div className="institution-evolution-container">
        <div className="institution-evolution-controls" style={{ marginBottom: '16px', textAlign: 'right' }}>
          <Space>
            <Button
              type="primary"
